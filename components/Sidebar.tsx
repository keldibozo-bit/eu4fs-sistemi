...
function ClusterNav({ lotId, cluster }: { lotId: string; cluster: Cluster }) {
...
function LotNav({ lot }: { lot: LotWithClusters }) {
  const [open, setOpen] = useState(false);
  return (
    <li>
      <button
      ...
      {open && (
        <ul className="ml-2 mt-0.5 mb-1 space-y-0.5 border-l border-slate-700 pl-3">
          {lot.clusters.length === 0 ? (
            ...
            lot.clusters.map((c) => <ClusterNav key={c.id} lotId={lot.id} cluster={c} />)
          )}
          <li>
            <Link
            ...
            {lotetOpen && (
              <ul className="mt-0.5 mb-1 space-y-0.5">
                {lots.length === 0 ? (
                  ...
                  lots.map((lot) => <LotNav key={lot.id} lot={lot} />)
                )}
              </ul>
            )}
          </li>
